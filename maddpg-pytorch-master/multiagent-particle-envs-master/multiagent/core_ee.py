...
class World(object):
    def __init__(self):
        ...
        self.length = 300  # m 所考虑区域的长度
        self.width = 300  # m 宽度 1000m×1000m的方形区域
        self.a = 4.88
        self.b = 0.43  # 环境变量
        ...
        self.data_rate = 0   # 初始化该回合的总数据传输速率为0
    ...
    def step(self):
        ...
        for agent in self.agents:
            self.update_agent_pos(agent)  # 新加
    ...
    # 自定义——更新agent位置坐标
    def update_agent_pos(self, agent):
        noise = np.random.randn(*agent.action.u.shape) * agent.u_noise if agent.u_noise else 0.0
        agent.action.u += noise
        ...
        # 分别更新agent的速度 横纵坐标 u[0]是方向 u[1]是速度 u[2]是调度的设备编号
