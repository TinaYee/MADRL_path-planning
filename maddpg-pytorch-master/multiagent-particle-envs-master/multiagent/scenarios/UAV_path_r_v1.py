...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):
        ...
        des = [world.length, world.width]  # 无人机降落的终点坐标
        delta_pos = agent.state.p_pos[:2] - des
        dis = np.sqrt(np.sum(np.square(delta_pos)))  # 无人机距离终点的距离
        max_dis = np.sqrt(world.length ** 2 + world.width ** 2)
        rew -= dis / max_dis  # 作为惩罚项
    ...
    def observation(self, agent, world):
        ...
        obs = np.hstack((np.ravel(agent.state.p_pos), np.ravel(entity_pos), np.ravel(comm), np.ravel(other_pos)))
    ...
    def obs_normal(self, agent, world, obs):
        for i in range(len(obs)):
            ...
            elif len(obs) - 1 >= i >= len(obs) - len(world.agents) + 1:  # 与其他无人机之间的距离
                obs[i] /= max_dis
            else:  # UE的调度状态
                obs[i] = obs[i]
        return obs
