...
from utils.agents import TD3Agent
...
class MATD3(object):
    ...
    def __init__(self, agent_init_params, alg_types,
                 gamma=0.95, tau=0.01, lr_a=0.01, lr_c=0.01, hidden_dim=64,
        ...
        self.agents = [TD3Agent(lr_a=lr_a, lr_c=lr_c,
        ...
        self.policy_noise = 0.3   # 策略噪声只需要比探索噪声稍大（1~2倍），最大值也不应超过 0.5
        self.noise_clip = 1  # 目标策略噪声的平滑范围
        self.policy_fre = 2  # new add——actor网络 延迟更新频率
    ...
    def step(self, observations, noise_std, explore=False):
        """
        ...
        """
        return [a.step(obs, noise_std, explore=explore) for a, obs in zip(self.agents, observations)]

    def update(self, sample, agent_i, t, parallel=False, logger=None):
        """
        ...
        if self.alg_types[agent_i] == 'MATD3':
            if self.discrete_action: # one-hot encode action
                ...
                # new add 给下一时刻状态s'的输出动作添加高斯噪声
                all_trgt_acs = []
                for pi, nobs in zip(self.target_policies, next_obs):
                    next_action = pi(nobs)  # 获取目标actor网络输出的动作
                    noise = torch.tensor(np.random.normal(0, self.policy_noise, size=next_action.shape),
                                         dtype=torch.float32)  # 生成高斯噪声
                    noisy_action = (next_action + noise).clamp(-1, 1)  # 添加噪声并限制在[-1, 1]范围内
                    all_trgt_acs.append(noisy_action)
