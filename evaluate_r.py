            ...
            if t_i == config.episode_length - 1 or all(dones):
                if ep_i == config.n_episodes -1:  # 最后一回合 存储该回合所有UE的位置坐标
                    for l in env.world.landmarks:
                        UE_pos.append([l.state.p_pos[0], l.state.p_pos[1], l.state.is_connected])
...
def save_plot():  # 保存数据&画图
    # 将每一回合的reward存为csv文件
    file_name1 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/UE_random/reward_e.csv'
    # save_data(file_name1, total_r)

    # 存储最后一回合的无人机轨迹
    file_name2 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/UE_random/UAV_path.csv'
    save_data(file_name2, UAV_pos)

    # 存储最后一回合的地面设备位置坐标
    file_name3 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/UE_random/UE_pos.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env_id", default="UAV_path_r", help="Name of environment")
    parser.add_argument("--model_name",
                        default="E:\lu\orientation\code\maddpg-pytorch-master\model\path",
                        help="Name of model")
    parser.add_argument("--run_num", default=12, type=int)
