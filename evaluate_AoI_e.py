...
from multiagent.core_aoi import World, Agent, Landmark
import pandas as pd
        ...
        for t_i in range(config.episode_length):
            ...
            if ep_i == config.n_episodes - 1:
                ...
                for i, a in enumerate(env.world.agents):
                    ...
                    for j, l in enumerate(env.world.landmarks):
                        if j == a.state.c[0]:
                            pos.append(l.state.aoi)
                            break
                    if a.state.c[0] == len(env.world.landmarks):
                        pos.append(0)
                UAV_pos.append(pos)

            # 本次回合结束 or 满足终止条件 [:]表示选取整个数组
            if t_i == config.episode_length - 1 or all(dones):
                if ep_i == config.n_episodes - 1:  # 最后一回合 存储该回合所有UE的位置坐标 和 其数据类型
                    for l in env.world.landmarks:
                        UE_pos.append([l.state.p_pos[0], l.state.p_pos[1], l.state.type, l.state.aoi])
                ...
                print('Episode:', ep_i, '\t|\t', 'Reward:', total_r[-1],
                      'AoI:', env.world.AoI, 'energy:', env.world.energy,
...
def save_plot():  # 保存数据&画图
    # 将每一回合的reward存为csv文件
    file_name1 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/aoi_e/reward_e.csv'
    # save_data(file_name1, total_r)

    # 存储最后一回合的无人机轨迹
    file_name2 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/aoi_e/UAV_path.csv'
    save_data(file_name2, UAV_pos)

    # 存储最后一回合的地面设备位置坐标
    file_name3 = 'E:/lu/orientation/code/maddpg-pytorch-master/results/aoi_e/UE_pos.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env_id", default="UAV_path_aoi_e", help="Name of environment")
    parser.add_argument("--model_name",
                        default="E:\lu\orientation\code\maddpg-pytorch-master\model\path_aoi_e",
                        help="Name of model")
